\clearpage
\section{学習モデル}
本章では、ハードウェアに実装するモデルの学習方法について説明する。
\subsection{学習モデルの概要および工夫点}
学習モデルの概要を\fref{fg:GANmodel}に示す。モデルは基本的にDCGAN\cite{radford_unsupervised_2016}の構成を参考にした。学習モデルの特徴は以下の二点である。
\begin{enumerate}
	\item 固定ベクトルを用いることで、新たな潜在空間を生成し、一つのネットワークで複数の潜在空間の学習に挑戦したこと
	\item カラー画像の精度向上のため、二種類のDiscriminetorを用いたこと
\end{enumerate}

\subsubsection*{Generator}
Generatorの入力は100次元の潜在ベクトル$z$、出力は$32\times 32$の画像である。ここで、一つのネットワークでRGB(赤緑青)の画像空間を学習させるために、値が固定の100次元の固定ベクトル$v_1$、$v_2$を用いる。具体的には、基本となる潜在空間$z$に対して、$z+v_1、z+v_2$の平行移動した潜在空間を用意し、$z$はR画像空間、$z+v_1$はG画像空間、$z+v_2$はB画像空間の学習入力に対応付ける。固定ベクトルを用いた、新たな空間生成の理論については、補足資料1にて検証を行う。

\subsubsection*{Discrminator}
Discriminatorでは、32×32の画像を見てそれが偽物（$G(z)$）か本物（正解画像）かの確率を0～1で出力する（本物:1、偽物:0）。本システムでは、カラー画像の判別精度向上のため、2種類のDiscriminatorを用意した。

Discrininator greyでは、「Generatorの出力するグレースケール画像（R,G,B）」に対し、「グレースケールの正解画像」と比較を行うことでアバターの形としての正しさを評価する。

Discriminator colorでは、「$z、z+v1、z+v2$の入力で得たR・G・B画像を用いて作成した偽物のカラー画像」と「正解カラー画像」との比較で正誤の確率を出力する。このように、Discriminatorの役割をアバターの形と色に分担を行うことで精度の向上を図った。

\begin{figure}[H]
	\centering
	\includegraphics[width=70mm,keepaspectratio]{figure/GANmodel.png}
	\caption{学習モデルの概要図}
	\label{fg:GANmodel}
\end{figure}
...
\subsection{FPGA実装に向けた、学習モデル構想の工夫（固定ベクトルの強み）}
...
GeneratorのFPGA実装にあたり、カラー画像生成にはR・G・B各成分に対応した学習が必要である。しかし、色ごとに別々のネットワークを学習した場合、3種類の重みパラメータを個別に保持・転送する必要があり、実装負荷が大きい。(\fref{fg:RGB_train2})

\begin{figure}[H]
	\centering
	\includegraphics[width=60mm,keepaspectratio]{figure/RGB_train2.png}
	\caption{RGBを別々のネットワークで学習した場合}
	\label{fg:RGB_train2}
\end{figure}

そこで、本システムでは固定値の100次元ベクトル$v_1, v_2$を導入し、潜在変数$z$から$z$、$z+v_1$、$z+v_2$の3種の入力を生成する。同一のネットワークに対して入力を切り替えることで、それぞれR画像、G画像、B画像の学習を行う（\fref{fg:RGB_train}）。
...
学習後は、Generator のパラメータおよび固定ベクトル $v_1, v_2$ を FPGA 上に実装することで、任意の 100 次元入力 $z$ に対し、FPGA 内部で $z$、$z+v_1$、$z+v_2$ を作成し、それぞれ R・G・B 成分に対応した画像を生成できる（\fref{fg:RGB_FPGA}）。

利用者目線では、任意の$z$入力を与えるだけで、R画像だけではなく、R、G、Bの三種類のデータを取得できる点が本システムの強みである。
...
\subsection{ネットワーク構成}
今回の学習モデルで用いたそれぞれのネットワークの構成を\tref{tab:network_io}に示す。各ネットワークの構造については、4.3.1～4.3.3 節で詳細に説明する。
\begin{table}[H]
	\centering
...
\subsubsection{Generator}
Generatorのネットワーク構造を\fref{fg:Gen}に示す。本ネットワークは、入力として100次元の潜在変数$z$を受け取り、逆畳み込み層(ConvTranspose2d)を用いて段階的に空間解像度を拡大する構造を有する。具体的には、1×1の潜在特徴マップを 4×4、8×8、16×16と段階的にアップサンプリングし、最終的に32×32の画像を生成する。各逆畳み込み層の後にはReLU関数を適用し、非線形性を導入することで表現能力を高めている。最終層ではTanh関数を用い、出力値を [-1,1] の範囲に正規化する。
...
\subsubsection{潜在空間の学習の確認}
100次元潜在ベクトル$z$は、平均0、分散1の独立な正規分布$\mathcal{N}(0,1)$ に従う乱数として生成される。新たにサンプリングされる$z_1, z_2$も同様の正規分布から生成されている。つまり、Generator はこの連続な潜在空間$z$から画像空間への連続写像$G(z)$を学習することで、多様なアバター画像を生成していると解釈できる。
...
% \fref{fg:hokan}の結果から、GANでは入力$z$に対して「ピクセル一致の正解画像」を学習しているのではなく、$z$という連続空間に対して「アバターらしい画像」を学習していることが分かる。また、隣接する画像を比較すると形状が似ているものが多く、画像を確率分布として学習していることが分かる。

\fref{fg:hokan}の結果から、GANでは入力$z$に対して「ピクセル一致の正解画像」を学習しているのではなく、$z$という連続空間に対して「アバターらしい画像」を確率分布として学習していることが分かる。
...
画素単位の一致を目的とした教師あり学習では、任意の2入力における補間点に対応する出力は、それぞれの教師画像の画素値を平均化したような結果になりやすく、視覚的にぼやけた画像が生成される傾向がある。一方、GAN は画像を確率分布としてモデル化し、潜在空間$z$から画像空間への写像$G(z)$を学習するため、任意の2つの潜在変数間の補間点においても、意味的に一貫性を保った新しい画像を生成することが可能である。この性質により、GAN は単なる画素補間では得られない多様な画像を生成でき、データセット拡張の観点において優位性を有するといえる。
