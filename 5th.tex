...
本システムにおけるGenerator回路は、画像生成のための大規模な転置畳み込み演算を効率的に実行するため、明確な役割分担を持つ複数のモジュールによる階層構造を採用している。また、データ転送効率向上及び演算時間短縮の観点から、4つの演算ユニットkernel\_multipleによる並列処理を採用している。さらに、リソース削減の観点から、重み格納メモリWeight\_RAMの定期更新と内部メモリの循環を採用している。各メモリ帯域および演算コア間の詳細なデータフローを\fref{fig:hierarchy_dataflow}に示す。
...
\begin{enumerate}
    ...
    外部から1クロックで一括して供給される100次元のランダムデータ（16bit $\times$ 100要素）を受け取るインターフェースである。1600bitの並列データを、本システムの演算単位である64bit（16bit $\times$ 4要素）ずつに分割してFeature\_RAMへ順次転送する。

    \item \textbf{Feature\_RAM / Weight\_RAM}:
    演算に必要なデータを保持するメモリである。Feature\_RAMは入力特徴マップを保持し、Weight\_RAMは学習済みの重みパラメータを保持する。4要素分の並列処理に対応するため、Feature\_RAMは64bit幅（16bit $\times$ 4要素）で構成されている。Weight\_RAMは、1クロックごとに8bitの重みが4つまとめて転送されてくるので、32bit幅（8bit $\times$ 4要素）を採用している。

    \item \textbf{deconv\_layer}:
    本システムの演算の中核を担うモジュールであり、内部に4基の演算コア（kernel\_multiple 1～4）を実装している。第1層から第4層までの転置畳み込み演算を担当し、メモリからデータを読み出して積和演算を実行する。

    \item \textbf{Accumulator\_RAM}:
    積和演算の途中経過を保持する累積加算用メモリである。演算過程におけるオーバーフローを防止するため、1要素あたり32bit幅を採用した。本設計ではdeconv\_layer内の4基の演算コアからの出力を同時に受け入れるため、128bit幅（32bit精度 $\times$ 4要素）を確保している。

    \item \textbf{Saturation}:
    Accumulator\_RAMから読み出された128bitの演算結果に対し、飽和演算を行いながら64bit幅へ丸め処理を行うモジュールである。
    
    \item \textbf{ReLU}:
    中間層（第1～3層）において使用される活性化関数モジュールである。負の値を0にする非線形処理を行う。

    \item \textbf{Tanh\_LUT}:
    最終層（第4層）において使用される活性化関数モジュールである。双曲線正接関数の値を格納したルックアップテーブルを参照することで、複雑な非線形計算を回避しつつ、データを画像の画素値に適した32bit範囲（8bit $\times$ 4要素）へ変換する。
...
演算フェーズでは、各層に対応するdeconv\_layerが順次起動される。deconv\_layerは、Feature\_RAMおよびWeight\_RAMから必要なデータを読み出し、内部演算コアkernel\_multipleへ供給する。kernel\_multipleでは、1クロックで4要素分の積和演算が実行される。演算された積和結果は、Accumulator\_RAMに対して順次加算され、畳み込み結果が形成される。
...
\begin{enumerate}
    ...
    \item \textbf{チャネル演算実行（Compute\_Channel）}: deconv\_layerに対して開始信号startを発行し、4つの演算コアを並列動作させ、1出力チャネル分の積和演算を実行する。
