...
\subsubsection{VAE}
% 画像データ$x$の特徴を潜在変数として次元の圧縮を行い、その潜在変数に基づいて画像を生成する仕組みをオートエンコーダ―(AE)という。
画像データ$x$を低次元の潜在表現に写像し、その潜在表現から元の画像を再構成する仕組みをオートエンコーダ(AE)という。AEでは潜在変数と出力画像は一対一対応だが、潜在変数を確率分布とすることで未知データの出力を可能とした技術としてVAE(Variational Auto Encoder)\cite{kingma_auto-encoding_2022}がある。(\fref{fg:VAE})
...
VAEでは$p(x|z)$をガウス分布としてモデル化するため、再構成誤差は二乗誤差に一致する。しかし、二乗誤差は正解画像との差異をとるため、新しく生成した画像を評価する際に、高周波成分を正確に再現する鋭い画像よりも、平滑化された曖昧な画像の方が誤差が小さくなりやすい傾向がある。その結果、VAEでは生成画像がぼやけやすいというアルゴリズム上の課題が存在する。

% VAEでは$p(x|z)$をガウス分布としてモデル化するため、再構成誤差は二乗誤差に一致する。しかし、二乗誤差は画素ごとの正解画像との差異を評価する指標であるため、新規に生成された画像を評価する際、高周波成分を正確に再現した鋭い画像よりも、平滑化された曖昧な画像の方が誤差が小さくなりやすい傾向がある。その結果、VAEでは生成画像がぼやけやすいというアルゴリズム上の課題が指摘されている。

% 多峰的な分布に対して平均化を促す性質がある。その結果、高周波成分を正確に再現する鋭い画像よりも、平滑化された曖昧な画像の方が誤差が小さくなりやすく、VAEでは生成画像がぼやけやすいというアルゴリズム上の課題が存在する。
...
VAEと異なり、GAN\cite{goodfellow_generative_2014}は潜在空間の推定を明示的に行わず、暗目的に$z\sim \mathcal{N}(0,I)$として潜在空間を定義する。そして、生成器の出力$G(z)$に対して、識別器が正解データと生成分布の差異を評価することで、画像の特徴を捉えた画像生成の方法を学習する。(\fref{fg:GAN_VAE})

% 明示的な尤度関数$p_\theta(x)$や潜在変数の事後分布$q(z|x)$を定義せず、識別モデルを介して生成分布とデータ分布との差異を評価することで、分布全体を暗黙的に近似する生成モデルである。(\fref{fg:GAN_VAE})
...
Discriminatorの目標は$p_{\mathrm{data}}$と$p_g$を判別することであるから、Fake=0, True=1とした場合、$D(G(z))=0,D(x)=1$となる。\eref{eq:BCE}を参考にすると、\eref{eq:LD}の損失は次式で表せる。
\begin{align}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=60mm,keepaspectratio]{figure/G_train.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=45mm,keepaspectratio]{figure/CNN4.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=45mm,keepaspectratio]{figure/CNN5.png}
	\caption{ゼロパディング}
	\label{fg:CNN5}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=45mm,keepaspectratio]{figure/CNN6.png}
